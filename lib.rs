...
#[openbrush::implementation(Ownable, PSP37, PSP37Metadata)]
#[openbrush::contract]
pub mod dropspace_sale {
    ...
    // MetaData for only one unique token ID
    use openbrush::{
        contracts::{
            ownable::{self},
            psp37:{
                self,
                extension::metadata::{self},
                Id,
            }
        },
        storage::Mapping,
    ...
    impl Contract {
        ...
        pub fn new(
            ...
            // ensure unique id on collection
        ...
        #[ink(message)]
        pub fn block(&mut self, id: Id) {
            self.blocked_ids.insert(&id, &());
        }

        fn mint_tokens(&mut self, id: Id, amount: Balance) -> Result<(), PSP37Error> {
            if self.blocked_ids.get(&id).is_some() {
                return Err(PSP37Error::Custom(String::from("Blocked Id exist")))
            }
        ...
        #[ink(message)]
        pub fn reserve(&mut self, reserve_datas: Vec<(Id, Amount)>) -> Result<(), PSP37Error> {
            if self.id_list < self.supply_limit {
        ...
        /*  Buy Token
        *   
        * ensure that id_list is not larger than supply_limit
        *    
        *
        */
        #[ink(message, payable)]
        pub fn buy(&mut self, buy_data: Vec<{Id, Balance}>) -> Result<(), PSP37Error> {
            let amount = buy_data.len() as u128;
            ...
            if self.id_list.saturating_add(amount) > self.supply_limit {
                return Err(PSP37Error::Custom(String::from(
            ...
            for (id, amount) in buy_data.iter() {
                let __ = self.mint_token(id.clone(), amount.clone());
            }
...
mod tests {
    ...
    fn buy_works() {
        ...
        assert_eq!(psp37::PSP37::total_supply(&contract), qty);

        assert_eq!(
            psp37::PSP37::owner_of(&contract, Id::U128(1)),
            Some(accounts.bob)
        );
        assert_eq!(psp37::PSP37::owner_of(&contract, Id::U128(qty)), None); // invalid token's ownership test
        ...
        // Transfer test
        assert_eq!(
            psp37::PSP37::owner_of(&contract, Id::U128(0)),
            Some(accounts.bob)
        );

        assert_eq!(
            psp37::PSP37::transfer(&mut contract, accounts.frank, Id::U128(0), Vec::new()),
            Ok(())
        );
    ...
    fn withdraw_works() {
        ...
        assert_eq!(contract.toggle_sale_active(), Ok(()));
        ink::env::test::set_caller::<ink::env::DefaultEnvironment>(accounts.bob); // Setting the caller for the next contract call                                                                               ink::env::pay_with_call!(contract.buy(vec![(Id::U8(1), 100), (Id::U8(2), 200)]), 2020),                                                                                  Ok(()));

        assert_eq!(
            ink::env::pay_with_call!(contract.buy(vec![(Id::U8(1), 100), (Id::U8(2), 200), (Id::U8(3), 300)]), 3030),
            Ok(())
        );
